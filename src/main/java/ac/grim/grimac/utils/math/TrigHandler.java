...
import static ac.grim.grimac.utils.math.fastmath.LegacyFastMath.getLegacyMathMovement;
...
public class TrigHandler {
    ...
    public void setOffset(Vector oldVel, double offset) {
        ...
        if (offset > 1e-5) {
            ...
            Vector legacyMath = getLegacyMathMovement(trueMovement, 0.1f, player.xRot);
            ...
            legacyMath = new Vector(Math.abs(legacyMath.getX()), 0, Math.abs(legacyMath.getZ()));
            ...
            double minLegacyHorizontal = Math.min(legacyMath.getX(), legacyMath.getZ());
            minLegacyHorizontal = Math.min(minLegacyHorizontal, Math.abs(legacyMath.getX() - legacyMath.getZ()));
