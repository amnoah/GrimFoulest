...
import ac.grim.grimac.utils.math.fastmath.FastMathType;
...
public class PredictionEngineElytra extends PredictionEngine {
    ...
    public List<VectorData> applyInputsToVelocityPossibilities(GrimPlayer player, Set<VectorData> possibleVectors, float speed) {
        ...
        for (VectorData data : possibleVectors) {
            Vector elytraResult = getElytraMovement(player, data.vector.clone(),
            ...
            // We must bruteforce OptiFine FastMath
            if (player.trigHandler.getActiveFastMath() == FastMathType.OPTIFINE_MATH) {
                elytraResult = getElytraMovement(player, data.vector.clone(),
                        ReachUtils.getLook(player, player.xRot, player.yRot))
                        .multiply(player.stuckSpeedMultiplier)
                        .multiply(new Vector(0.99F, 0.98F, 0.99F));
            }
